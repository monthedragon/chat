...
if ($user_type == ADMIN_CODE) {
	...
	echo '<hr>';
	echo "<input type='input' id='user_chat_search' name='user_chat_search' style='width:80%;margin-bottom:10px;' placeholder='search name here'>";
}
...
foreach ($users as $details) {
	...
	echo "<div class='div-user {$fullNameCls}' style='margin:1px;display:flex; text-align:left align-items: center;'>";
	echo "{$checkbox}<span style='display:inline-block;height:15px;width:80%;margin-top:5px;' class='div-user-chat cursor-pointer {$unread_cls}' user_id='{$details['user_name']}' >{$fullName}</span>";
	echo "</div>";
	// echo '<br>';
}
echo '</div>';
...
<script>
	...
	$(function() {
		$(".div-user-chat, .div-gc-chat").mouseover(function() {
			$(this).addClass('highlight_cls');
		})
		$(".div-user-chat, .div-gc-chat").mouseout(function() {
			$(this).removeClass('highlight_cls');
		})
		...
		$('#user_chat_search').on('input', function() {
			var wildclass = $(this).val().toLowerCase();

			// Hide all div elements initially
			$('.div-user').hide();

			// Show div elements whose class attribute matches the wildcard pattern
			$('.div-user').filter(function() {
				return new RegExp(wildclass).test($(this).attr('class'));
			}).show();
		});
